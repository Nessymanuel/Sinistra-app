  ...
  return (
    <section className="section-card max-w-5xl mx-auto space-y-4">
      <div className="flex items-center justify-between gap-2">
        ...
        <span className="text-[11px] text-slate-500">{claims.length} registo(s)</span>
      </div>
        ...
        <table className="min-w-full border border-slate-200 rounded-lg overflow-hidden">
          <thead className="bg-slate-50">
            <tr>
              ...
              <th className="px-3 py-2 text-left font-semibold text-slate-700">Data / Local</th>
              <th className="px-3 py-2 text-left font-semibold text-slate-700">Veículo</th>
              <th className="px-3 py-2 text-left font-semibold text-slate-700">Seguradora</th>
              <th className="px-3 py-2 text-left font-semibold text-slate-700">Feridos</th>
              <th className="px-3 py-2 text-left font-semibold text-slate-700">Estado</th>
              <th className="px-3 py-2 text-left font-semibold text-slate-700">Criado em</th>
            </tr>
          </thead>
          <tbody>
            {claims.map(function (c, index) {
              ...
              return (
                <tr key={c.id} className={zebra + " border-t border-slate-200"}>
                  <td className="px-3 py-2 font-semibold text-slate-900">#{c.id}</td>
                  <td className="px-3 py-2">
                    <div className="flex flex-col">
                      <span>{c.dataAcidente}</span>
                      <span className="text-[11px] text-slate-500">{c.local}</span>
                    </div>
                  </td>
                  <td className="px-3 py-2">
                    <span className="text-[11px]">{c.matricula || "—"}</span>
                  </td>
                  <td className="px-3 py-2">
                    <span className="text-[11px]">{c.seguradora || "—"}</span>
                  </td>
                  <td className="px-3 py-2">
                    <span
                      className={
                  ...
                  <td className="px-3 py-2 text-[11px] text-slate-500">{c.criadoEm}</td>
                </tr>
              );
            })}
          </tbody>
